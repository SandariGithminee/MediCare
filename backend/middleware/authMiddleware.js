    ...
    try {
      ...
      try {
        ...
        if (!authError && authData?.user) {
          ...
          const { rows } = await db.query(
            "SELECT id, auth_id, name, email, role, created_at FROM users WHERE auth_id = $1 OR email = $2",
          ...
          if (rows.length > 0) {
            ...
            const insertResult = await db.query(
              "INSERT INTO users (auth_id, name, email, role) VALUES ($1, $2, $3, $4) RETURNING id, auth_id, name, email, role",
      ...
      if (process.env.JWT_SECRET) {
        try {
          ...
          const { rows } = await db.query(
            "SELECT id, auth_id, name, email, role, created_at FROM users WHERE id = $1",
            [decoded.id]
          );
